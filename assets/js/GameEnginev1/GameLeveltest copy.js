  ...
  constructor(gameEnv) {
    ...
    const meteorReaction = () => {
      ...
      setTimeout(() => {
        ...
        if (player._gravityVelocity !== undefined) player._gravityVelocity = 0;
    ...
    // ── Gravity constants ─────────────────────────────────────────────────────
    const GRAVITY    = 0.5;   // pulls player down every frame
    const FLAP_FORCE = -10;   // upward burst when W is pressed
    const MAX_FALL   = 12;    // terminal velocity cap

    // ── Player ────────────────────────────────────────────────────────────────
    const playerData = {
      id: 'player-test',
      greeting: 'Use W to flap, A/D to move sideways',
      src: spriteSrc,
      SCALE_FACTOR: 8,
      STEP_FACTOR: 600,
      ANIMATION_RATE: 50,
      ...
      update: function() {
        if (this._gravityVelocity === undefined) this._gravityVelocity = 0;

        // W = flap — only triggers on fresh press, not while held
        const wDown = this.pressedKeys?.[87];
        if (wDown && !this._wWasDown) {
          this._gravityVelocity = FLAP_FORCE;
        }
